...
Before the pipeline ever runs, `detect_doc_kind` checks the saved file's
actual content (user decision: only PDF and Word `.docx` are supported —
see `upload.html`'s note and the `accept` attribute on the file input).
An unsupported file is removed from `inbox/` right away — never archived
into `sources/`, never run through the pipeline — and a warm, plain-language
error names the file's type and what a-doc does accept. A file that IS a
supported type is ingested with `inbox_root=inbox_dir`, opting it into
`ingest.pipeline`'s post-ingest inbox hygiene (deleted on success/duplicate,
moved to `work/failed/` + logged on error — see that module's docstring).
"""
...
async def upload_submit(
    ...
    if detect_doc_kind(dest) is None:
        dest.unlink(missing_ok=True)
        return templates.TemplateResponse(
            request,
            "upload.html",
            {"report": None, "error": _unsupported_file_message(filename)},
        )
