...
- App CLI: `uv run adoc <init|onboard|ingest|review|serve|backfill|backfill-doc-text|eval|user|identifiers|backup|restore|bootstrap-data|labs-infer-specimen|labs-dedupe-twins|labs-reclassify|labs-recanonicalize|intake-corroborate|regimen-backfill|phenotype-backfill|intake-replay>` (21 subcommands; `user` has `add`/`list`/`remove`; `identifiers` has `show`/`add`/`remove`)
