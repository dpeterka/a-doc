...
| Privacy | API only (never consumer apps); direct identifiers (name/DOB/MRN/address) stripped from all text paths in `privacy.py`; vision extraction necessarily sends raw documents. Avoid retention-extending API features (Files/Batch) in v1. |
...
10. Document drop — existing PDFs/scans, worked into the conversation rather than a fixed final step. Auto-covered when `sources/` is already non-empty (a seeded/curated deployment, or a local repo that already ran a backfill).
...
- **Claim-level entailment verifier** (ADR 0016): a third model family (`entailment_verifier` role, Featherless/DeepSeek — distinct from `primary_reasoner`/Anthropic and `challenger`/OpenAI, per ADR 0005's cross-family rule) judges `(claim, resolved source text)` pairs `entailed`/`not_entailed`/`insufficient_source` via `reason/verify.py`. `not_entailed` claims get one retry with the verifier's objection, then an `entailment_check_*` DAG contract fires. Source-text resolution is an injectable `SourceTextResolver`: `labs:` refs resolve from the stored row, `encounter:` refs from the encounter file, `doc:` refs from the document-text corpus (ADR 0015, page-scoped when the ref names a page); a document with no extracted text yields `insufficient_source`, never a rejection. `pmid:`/`patient-report:` refs stay unresolved (no external source to entail against). Composer output gets an equivalent deterministic pass: `check_composer_numbers` extracts every number near a known analyte name and requires it to match a stored value, wired as a `composer_number_check` DAG contract.
