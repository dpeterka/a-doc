...
10. Document drop — prompt to put existing PDFs in the Dropbox folder / connect Apple Health portals. Auto-skipped when `sources/` is already non-empty (a seeded/curated deployment restored from S3, or a local repo that already ran a backfill) — nothing to ask the patient to upload that is already on file; a genuinely fresh, empty data repo still gets the prompt.
